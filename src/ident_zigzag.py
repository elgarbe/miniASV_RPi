...
class IdentZZNode(object):
    ...
    def __init__(self):
        ...
        if not os.path.exists(path):
            os.makedirs(path)
        self.ident_file = open('%s/ident-zz.log' % path,'w')
        ...
        rospy.loginfo('[IDNTZZ] Subscribing to ASV velocity topic: %s', velocity_topic)
        ...
        rospy.loginfo('[IDNTZZ] Will publish PWM command to topic: %s', cmd_pwm_topic)
        ...
        rospy.loginfo('[IDNTZZ] Update rate: %f Hz', self.rate)

        rospy.loginfo("[IDNTZZ] Start Identification Node ...")
    ...
    def start(self, x):
        rospy.loginfo("[IDNTZZ] Comenzando auto reference")
    ...
    def shutdown(self):
        ...
        rospy.loginfo("[IDNTZZ] Sayonara generador de referencia. Nos vemo' en Disney.")

def main():
    """Entrypoint del nodo"""
    rospy.init_node('ident_zz', anonymous=True, log_level=rospy.INFO)
    node = IdentZZNode()

    try:
        rospy.spin()
    except KeyboardInterrupt:
        rospy.loginfo("[IDNTZZ] Received Keyboard Interrupt (^C). Shutting down.")
