...
class ReferenceNode(object):
    ...
    def __init__(self):
        ...
        self.sub_ref_y   = rospy.Subscriber('ref_y', Float32, self.set_ref_y)

        # Creo el Publisher para la referencia
        self.reference_publisher = rospy.Publisher(reference_topic, CourseStamped, queue_size=10)
        rospy.loginfo('[REF] Will publish Course reference to topic: %s', reference_topic)
    ...
    def publish(self, event):
        ...
        message.speed = self.ref[0]
        message.yaw = self.ref[1]
        # Publico el mensaje
    ...
    def shutdown(self):
        ...
        self.sub_ref_y.unregister()
        rospy.loginfo("[REF] Sayonara generador de referencia. Nos vemo' en Disney.")

def main():
    """Entrypoint del nodo"""
    rospy.init_node('asv_ref_course', anonymous=True, log_level=rospy.INFO)
    node = ReferenceNode()
